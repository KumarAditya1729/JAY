...
class CommitmentMonitor:
    ...
    def analyze(self) -> list[OpenCommitment]:
        ...
        items = self.session.query(MemoryItem).filter(MemoryItem.kind == "commitment").all()
        for item in items:
            ...
            if item.occurred_at:
                occurred = item.occurred_at.replace(tzinfo=timezone.utc) if item.occurred_at.tzinfo is None else item.occurred_at
                if occurred < now:
            ...
            commitments.append(OpenCommitment(
