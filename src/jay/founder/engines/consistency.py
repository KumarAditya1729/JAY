...
class ConsistencyEngine:
    ...
    def analyze(self) -> dict:
        intent_nodes = self.session.query(IntentNode).filter(IntentNode.status == "active").all()
        ...
        if intent_nodes and not behaviors and not decisions:
            gaps.append("Founder has stated goals/intents, but no actions or decisions have been recorded.")
            alignment_score -= 20
        ...
        if unaligned_decisions:
            ...
            if ratio > 0.3:
                gaps.append(f"{ratio*100:.0f}% of recent decisions are not aligned with Founder Intent.")
        ...
        if ignored and len(behaviors) > 0:
            ...
            if ratio > 0.5:
                gaps.append(f"Founder is ignoring {ratio*100:.0f}% of system recommendations.")
                alignment_score -= int(ratio * 20)
                
        return {
            "consistency_score": max(0, alignment_score),
            "identified_gaps": gaps if gaps else ["Founder is acting consistently with stated intent and goals."]
        }
