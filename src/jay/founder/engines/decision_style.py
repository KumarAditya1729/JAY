...
class DecisionStyleEngine:
    ...
    def analyze(self) -> dict:
        ...
        if successes and sum(len(d.evidence) for d in successes) / len(successes) > 2:
            success_patterns.append("Decisions with strong evidence consistently succeed.")
            
        failure_patterns = []
        if failures and sum(len(d.options_considered) for d in failures) / len(failures) < 1.5:
            failure_patterns.append("Failing decisions often lack alternative options.")

        return {
            "speed": "Fast (based on timestamp analysis against intent creation)" if len(decisions) > 10 else "Unknown",
