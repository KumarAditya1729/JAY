...
class RelationshipEngine:
    ...
    def analyze(self) -> list[Relationship]:
        ...
        people = (
            self.session.query(MemoryItem).filter(MemoryItem.kind == "person").all()
        )

        # Optimize by loading all meetings once
        meetings = (
            self.session.query(MemoryItem).filter(MemoryItem.kind == "meeting").all()
        )
        ...
        for person in people:
            ...
            last_contact = max(
                [m.occurred_at or m.created_at for m in person_meetings], default=None
            )

            if last_contact:
                last_contact = (
                    last_contact.replace(tzinfo=timezone.utc)
