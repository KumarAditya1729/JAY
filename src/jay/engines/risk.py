...
class RiskEngine:
    ...
    def analyze(self) -> list[Risk]:
        ...
        for c in commitments:
            if c.occurred_at:
                occurred = (
                    c.occurred_at.replace(tzinfo=timezone.utc)
                    if c.occurred_at.tzinfo is None
                    else c.occurred_at
                )
                if occurred < now:
        ...
        # 2. Project Risk
        projects = (
            self.session.query(MemoryItem).filter(MemoryItem.kind == "project").all()
        )
        for p in projects:
            latest_trust = (
                self.session.query(TrustLedger)
                .filter(TrustLedger.entity_id == p.id)
        ...
        decisions = (
            ...
            .filter(MemoryItem.kind.in_(["decision", "project"]))
            .all()
        )
        for d in decisions:
