...
class OpportunityEngine:
    ...
    def analyze(self) -> list[Opportunity]:
        ...
        dormant_items = self.session.query(MemoryItem).filter(
            MemoryItem.kind.in_(["idea", "project"]),
            MemoryItem.importance >= 4,
            MemoryItem.updated_at < fourteen_days_ago
        ).all()
        for item in dormant_items:
            updated = item.updated_at.replace(tzinfo=timezone.utc) if item.updated_at.tzinfo is None else item.updated_at
            if "completed" not in [t.lower() for t in item.tags] and "archived" not in [t.lower() for t in item.tags]:
        ...
        decisions = self.session.query(MemoryItem).filter(
            MemoryItem.kind == "decision",
        ...
        return sorted(opportunities, key=lambda x: (x.impact_score * x.confidence_score), reverse=True)
