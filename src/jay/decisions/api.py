...
def list_decisions(session: Session = Depends(get_session)):
    return session.query(DecisionLedger).order_by(DecisionLedger.decision_date.desc()).all()

@router.get("/timeline", response_model=list[DecisionResponse])
def decision_timeline(session: Session = Depends(get_session)):
    return session.query(DecisionLedger).order_by(DecisionLedger.decision_date.asc()).all()

@router.get("/{decision_id}", response_model=DecisionResponse)
def get_decision(decision_id: str, session: Session = Depends(get_session)):
    decision = session.query(DecisionLedger).filter(DecisionLedger.id == decision_id).first()
    if not decision:
